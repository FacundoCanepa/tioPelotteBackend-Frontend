  ...
  return (
    <header className="w-full flex items-center justify-between py-3 md:py-0 md:pl-5 md:pr-5 shadow-md relative z-50 navbar-secondary   ">
      ...
      <div className="flex flex-col items-start space-y-2  ">
        <span className={`text-[10vw]  md:text-[3vw] md:tracking-widest md:font-semibold font-island cursor-pointer`}  onClick={() => router.push ("/")} >Las pastas de tu pueblo</span> 
      </div>
      <div className="flex items-center space-x-3 md:space-x-10  md:pr-5">
        <button onClick={toggleMenu} >
          {isOpen ? <X className="w-[7vw] h-[7vw] md:w-[2vw] md:h-[2vw] cursor-pointer " /> : <Menu className="w-[7vw] h-[7vw] md:w-[2vw] md:h-[2vw] cursor-pointer" />}
        </button>
        <button>
          <ShoppingCart   className="w-[7vw] h-[7vw] md:w-[2vw] md:h-[2vw] cursor-pointer" onClick={() => router.push ("/carrito")} />
        </button>
        <button>
